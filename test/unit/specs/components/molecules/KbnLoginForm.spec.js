    ...
    describe('onlogin', () => {
      ...
      describe('resolve', () => {
        it('resolveされること', done => {
          ...
          expect(loginForm.vm.error).to.equal('')// エラーめっsーーじは初期化
      ...
      describe('reject', () => {
        it('rejetされること', done => {
          ...
          // クリックイベント
          loginForm.find('button').trigger('click')
          expect(onloginStub.called).to.equal(false)// mdあだrejectされない
          expect(loginForm.vm.error).to.equal('')// エラーメッセージは初期化
          expect(loginForm.vm.diaableLoginAction).to.equal(true)// ログイン悪村はふか
          // 状態の反映
          loginForm.vm.$nextTick(() => {
            ...
            expect(authInfo.password).to.equal(loginForm.vm.password)
            loginForm.vm.$nextTick(() => {
              expect(loginForm.vm.error).to.equal('login error!') // エラ〜メッセージが設定される
              expect(loginForm.vm.disableLoginAciton).to.equal(false)// ログインアクションは可能
              done()
            })
          })
        })

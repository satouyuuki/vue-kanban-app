...
const mockLoginAction = login => {
  // inject-loaderを使ってアクションないの依存関係をモックあするタッ目の注入関数を取得する
  const actionsInjector = require('inject-loader!@/store/actions')

  // 注入関数でAuth APIモジュールをモックかする
...
describe('loginアクション', () => {
  ...
  describe('Auth.loginが成功', () => {
    ...
    it('成功となること', () => {
      ...
      expect(commit.args[0][1].tooken).to.equal(token)
      expect(commit.args[0][1].userId).to.equal(userId)
    })
  })
  describe('Auth.loginが失敗', () => {
    beforeEach(done => {
      const login = authInfo => Promise.reject(new Error('login faled'))
      const action = mockLoginAction(login)
      commit = sinon.spy()
      // loginアクションの実行
